...
public:
	...
	static void CreateUnitSquareXZ(int side, int repeatU, int repeatV, VertexStride_VUN*& pVerts, int& nverts, TriangleIndex*& pTriList, int& ntri);

	static void CreateUnitEgg(int vslice, int hslice, VertexStride_VUN*& pVerts, int& nverts, TriangleIndex*& pTriList, int& ntri, float b_val = .75f, float w_val = .1f);

private:
	static float HugelschafferEgg(float B, float w, float x);
