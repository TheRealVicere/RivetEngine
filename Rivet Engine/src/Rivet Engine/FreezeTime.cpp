...
const float FreezeTime::DEAD_TIME_THRESHOLD = 0.2f;
...
float FreezeTime::ComputeGameTime(float sys_time, float prev_gametime)
{
	...
	float curr_gametime = sys_time - totalFrozenTime;
...
void FreezeTime::TestForFreezeKeys()
{
	...
	if (HackedKeyRelease(FREEZE_KEY) || timeIsFrozen)
	{
		DebugMsg::out("FREEZE FRAME at time %f (last frame: %f) ",
			GameTimer::GetElapsedTime(), GameTimer::GetFrameTime());

		timeIsFrozen = true;				// Freeze mode active
		...
		while (timeIsFrozen && !single_frame_requested)
		{
			if (HackedKeyRelease(FREEZE_KEY))
			{
				timeIsFrozen = false;		// Cancel both freeze modes
	...
	if (HackedKeyRelease(DEBUG_KEY))
		DebugAttorney::ToggleDebug();
}
